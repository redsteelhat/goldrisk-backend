...
router.post(
  ...
  (req: Request, res: Response): void => {
    ...
    if (!parent_transaction_id || !gold_item_id || quantity_g == null || !payment_method) {
      res.status(400).json({
        error: 'parent_transaction_id, gold_item_id, quantity_g, payment_method required',
      });
      return;
    }
    if (!req.user) {
    ...
    createReturn(
      {
        ...
        gold_item_id,
        quantity_g,
        labor_refund_amount: labor_refund_amount != null ? toTRY(labor_refund_amount) : undefined,
        payment_method,
        client_request_id,
        notes,
      },
...
/** POST /transactions/adjustment - manager onayı */
router.post(
  '/adjustment',
  requireOwnerOrManager,
  (req: Request, res: Response): void => {
    const { product_id, entry_type, quantity_g, unit_price_g, client_request_id, notes } =
      req.body;
    if (!product_id || !entry_type || quantity_g == null || unit_price_g == null) {
      res.status(400).json({
        error: 'product_id, entry_type, quantity_g, unit_price_g required',
      });
      return;
    }
    if (entry_type !== 'debit' && entry_type !== 'credit') {
      res.status(400).json({ error: 'entry_type must be debit or credit' });
      return;
    }
    if (!req.user) {
      res.status(401).json({ error: 'Unauthorized' });
...
/** POST /transactions/scrap */
router.post(
  '/scrap',
  requireRole('owner', 'manager', 'cashier'),
  (req: Request, res: Response): void => {
    const { gold_item_id, client_request_id, notes } = req.body;
    if (!gold_item_id) {
      res.status(400).json({ error: 'gold_item_id required' });
      return;
    }
    if (!req.user) {
      res.status(401).json({ error: 'Unauthorized' });
